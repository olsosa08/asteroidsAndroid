...
public class SmallAsteroid implements GraphicsObject{
    ...
    public void destroy(SmallAsteroid sAster, ArrayList<SmallAsteroid> mSAsterArray, Bullet mBullet){
        boolean temp = collision(sAster, mBullet);
        if (temp){
            mSAsterArray.remove(sAster);
        }
    }

    public boolean collision(SmallAsteroid mSAster, Bullet mBullet){
        boolean ans = false;
        double distance = Math.sqrt(Math.pow(mSAster.getX() - mBullet.getX(), 2) + Math.pow(mSAster.getY() - mBullet.getY(),2));
        if(distance < mSAster.getRadius()){
            ans = true;
